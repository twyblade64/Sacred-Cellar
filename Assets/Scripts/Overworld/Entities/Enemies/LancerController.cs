...
/// </summary>
public class LancerController : SmartEnemy, ITriggerReciever {
    ...
    public override bool CheckEnemyInAttackRange() {
        return (Mathf.Abs(player.transform.position.x - transform.position.x) <= attackRange && Mathf.Abs(player.transform.position.z - transform.position.z) <= .5)
            || (Mathf.Abs(player.transform.position.z - transform.position.z) <= attackRange && Mathf.Abs(player.transform.position.x - transform.position.x) <= .5);
    }
    ...
    public override bool Damage(float damage = 0, int direction = 0, float pushForce = 0, float stunForce = 0) {
        ...
        float pushForceSum =  Mathf.Max(0, pushForce - stats.pushResistance);
        Debug.Log("Push: " + pushForceSum);
        if (pushForceSum > 0) {
