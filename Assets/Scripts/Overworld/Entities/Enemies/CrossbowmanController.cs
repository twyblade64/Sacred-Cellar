...
/// <summary>
/// This enemy attacks the player with a crossbow from distance.
/// </summary>
public class CrossbowmanController : SmartEnemy, ITriggerReciever {
    private PlayerController player;
    private Animator animator;

    // Custom crossbowman data
    [Header("Custom Stats")]
    ...
    /// <summary>
    /// Initialize logic, load AudioClips into the SoundManager and create object pool of proyectiles.
    /// </summary>
    ...
    public override bool CheckEnemyInAttackRange() {
        ...
        if (!(dist.magnitude <= firingDistance))
            return false;
        float orthoDist;
        if (Mathf.Abs(dist.x) > Mathf.Abs(dist.z))
            orthoDist = Mathf.Abs(dist.z);
        else
            orthoDist = Mathf.Abs(dist.x);
        if (orthoDist > 0.25f)
            return false;
        return true;
